    ...
    <section class="relative bg-gradient-hero text-white py-20 lg:py-28 overflow-hidden">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-wrap items-center gap-4 text-gray-300 text-sm animate-fade-in-up animation-delay-100">
                @if ($newsEvent->department?->slug)
                    <a href="{{ route('view.departments.show', $newsEvent->department->slug) }}" class="hover:text-primary-300 transition-colors flex items-center gap-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                        {{ $newsEvent->department->name }}
                    </a>
                @elseif ($newsEvent->applies_to_all_departments)
                ...
                <div class="card-premium p-6 animate-fade-in-right">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="p-3 bg-gray-50 rounded-xl">
                            ...
                            <a href="{{ route('view.departments.show', $newsEvent->department->slug) }}" class="font-semibold text-maroon-600 text-sm hover:text-maroon-700 transition-colors">{{ $newsEvent->department->name }}</a>
                        </div>
                        @elseif ($newsEvent->applies_to_all_departments)
                        <div class="p-3 bg-gray-50 rounded-xl">
                            <p class="text-xs text-gray-400 font-medium uppercase tracking-wider">Department</p>
                            <p class="font-semibold text-maroon-600 text-sm mt-0.5">All Departments</p>
                        </div>
                        @endif
